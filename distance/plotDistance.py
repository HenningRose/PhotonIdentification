...
def Distributions(conversion, sgn_data, bkg_data, stage, plot_type, save_directory):
    """ Plots the pt and eta distributions to check reweighting!
    Variables:
    conversion: conversion type
    sgn_data: signal dataframe
    bkg_data: background dataframe
    stage: original (before reweighting) or reweight (after reweighting)
    plot_type: plot 1 or 2 plots per line in latex file
    save_directory: directory to save plot in
    """
    pt = [20, 25, 30, 35, 40, 45, 50, 60, 80, 100, 125, 150, 175, 250, 500, 1500]
    ...
    fig_pt, ax1_pt = plt.subplots(nrows=1, ncols=1, figsize=fig_size)
    fig_eta, ax1_eta = plt.subplots(nrows=1, ncols=1, figsize=fig_size)

    ax1_eta.tick_params(which='major', length=10)
    ax1_eta.tick_params(which='minor', length=5)
    ax1_eta.tick_params(axis='both', which='major', labelsize=10)
    ax1_eta.tick_params(axis='x', direction='in', bottom=True, labelbottom=True, top=True, labeltop=False, which='both')
    ax1_eta.tick_params(axis='y', direction='in', left=True, labelleft=True, right=True, labelright=False, which='both')
    ax1_eta.hist(sgn_data.y_eta.values, weights=sgn_data.train_weight.values, bins=eta, color="red", edgecolor='red', linewidth=1.5, alpha = 0.4, label="signal",  density=True, stacked=True)
    ax1_eta.hist(bkg_data.y_eta.values, weights=bkg_data.train_weight.values, bins=eta, color="blue", edgecolor='blue', linewidth=1.5, alpha = 0.4, label="background",  density=True, stacked=True) 
    ax1_eta.set_ylabel("weighted events", fontsize=label_size)
    ax1_eta.set_xlabel(r"$\eta$", fontsize=label_size)
    ax1_eta.set_xticks(eta)
    ax1_eta.set_xticklabels(eta, rotation=45)
    ax1_eta.set_yticks([])
    ax1_eta.set_xlim(0, 2.37)
    legend = ax1_eta.legend(title=f'{conversion} photons', fontsize=10, loc="best", facecolor='grey', edgecolor='black', framealpha=0.1)
    legend._legend_box.sep = 5
    ax1_pt.tick_params(which='major', length=10)
    ax1_pt.tick_params(which='minor', length=5)
    ax1_pt.tick_params(axis='both', which='major', labelsize=10)
    ax1_pt.tick_params(axis='x', direction='in', bottom=True, labelbottom=True, top=True, labeltop=False, which='both')
    ax1_pt.tick_params(axis='y', direction='in', left=True, labelleft=True, right=True, labelright=False, which='both')
    ax1_pt.hist(sgn_data.y_pt.values, weights=sgn_data.train_weight.values, bins=pt, color="red", edgecolor='red', linewidth=1.5, alpha = 0.4, label="signal",  density=True, stacked=True)
    ax1_pt.hist(bkg_data.y_pt.values, weights=bkg_data.train_weight.values, bins=pt, color="blue", edgecolor='blue', linewidth=1.5, alpha = 0.4, label="background",  density=True, stacked=True) 
    ax1_pt.set_ylabel("weighted events", fontsize=label_size)
    ax1_pt.set_xlabel(r"$E_T$ / GeV", fontsize=label_size)
    ax1_pt.set_xticklabels([], rotation=45)
    ax1_pt.set_yticks([])
    ax1_pt.set_xscale('log')
    ax1_pt.set_xlim(20, 1500)
    legend = ax1_pt.legend(title=f'{conversion} photons', fontsize=label_size, loc="best", facecolor='grey', edgecolor='black', framealpha=0.1)
    legend._legend_box.sep = 5

    #fig.tight_layout()
    fig_pt.savefig(f"/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{save_directory}/pt_{stage}.pdf", bbox_inches='tight')
    plt.close(fig_pt)
    fig_eta.savefig(f"/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{save_directory}/eta_{stage}.pdf", bbox_inches='tight')
    plt.close(fig_eta)
...
def Loss(conversion, model_history, lambda_value, save_directory):
    ...
    save_directory: directory to save plot in
    """
    string_lambda_value = str(abs(lambda_value))
    train_combined = model_history.history['loss']
    test_combined = model_history.history['val_loss']
    train_classifier = model_history.history['output_loss']
    test_classifier = model_history.history['val_output_loss']
    train_distance = model_history.history['output_1_loss']
    test_distance = model_history.history['val_output_1_loss']
    ...
    axs[0].tick_params(axis='x', direction='in', bottom=True, labelbottom=False, top=True, labeltop=False, which='both')
    axs[0].tick_params(axis='y', direction='in', left=True, labelleft=True, right=True, labelright=False, which='both')
    axs[0].plot(train_combined, color='cornflowerblue', lw=1.5, label='training')
    axs[0].plot(test_combined, color='darkorange', lw=1.5, label='validation')
    axs[0].set_ylabel(r'$L_{classifier}+ %s \cdot L_{dCorr}$' % (string_lambda_value), fontsize=label_size)
    #axs[0].set_xlabel('epochs', fontsize=label_size)
    axs[0].grid(True)
    #axs[0].set_title('classifier loss', fontsize=label_size)
    legend = axs[0].legend(title=f'{conversion} photons', fontsize=label_size, loc="best", facecolor='grey', edgecolor='black', framealpha=0.1)
    legend._legend_box.sep = 5
    ...
    axs[1].tick_params(axis='x', direction='in', bottom=True, labelbottom=False, top=True, labeltop=False, which='both')
    axs[1].tick_params(axis='y', direction='in', left=True, labelleft=False, right=True, labelright=True, which='both')
    axs[1].plot(train_classifier, color='cornflowerblue', lw=1.5, label='training')
    axs[1].plot(test_classifier, color='darkorange', lw=1.5, label='validation')
    axs[1].yaxis.set_label_position("right")
    axs[1].set_ylabel(r'$L_{classifier}$', fontsize=label_size)
    #axs[1].set_xlabel('epochs', fontsize=label_size)
    axs[1].grid(True)
    #axs[1].set_title('classifier loss', fontsize=label_size)
    #legend = axs[1].legend(title=f'{conversion} photons', fontsize=label_size, loc="best", facecolor='grey', edgecolor='black', framealpha=0.1)
    #legend._legend_box.sep = 5
    ...
    axs[2].tick_params(axis='x', direction='in', bottom=True, labelbottom=True, top=True, labeltop=False, which='both')
    axs[2].tick_params(axis='y', direction='in', left=True, labelleft=True, right=True, labelright=False, which='both')
    axs[2].plot(train_distance, color='cornflowerblue', lw=1.5, label='training')
    axs[2].plot(test_distance, color='darkorange', lw=1.5, label='validation')
    axs[2].set_ylabel(r'$L_{dCorr}$', fontsize=label_size)
    axs[2].set_xlabel('epochs', fontsize=label_size)
    axs[2].grid(True)
    #axs[2].set_title('classifier loss', fontsize=label_size)
    #legend = axs[2].legend(title=f'{conversion} photons', fontsize=label_size, loc="best", facecolor='grey', edgecolor='black', framealpha=0.1)
    #legend._legend_box.sep = 5

    #fig.tight_layout()
    plt.savefig(f"/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{save_directory}/loss.pdf", bbox_inches='tight')
    plt.close
...
def ROC(conversion, y_train, y_train_pred, w_train, y_test, y_test_pred, w_test, plot_type, save_directory):
    ...
    #fig.tight_layout()
    plt.savefig(f"/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{save_directory}/ROC.pdf", bbox_inches='tight')
    plt.close
...
def classifierPredictions(conversion, y_train, y_train_pred, w_train, y_test, y_test_pred, w_test, plot_type, save_directory):
    ...
    #fig.tight_layout()
    fig_train.savefig(f"/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{save_directory}/score_train.pdf", bbox_inches='tight')
    plt.close(fig_train)
    fig_test.savefig(f"/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{save_directory}/score_test.pdf", bbox_inches='tight')
    plt.close(fig_test)
...
def Efficiency(conversion, y_train, y_train_pred, w_train, tight_train, y_test, y_test_pred, w_test, tight_test, save_directory):
    ...
    fig.tight_layout()
    plt.savefig(f"/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{save_directory}/efficiency.pdf", bbox_inches='tight')
    plt.close()
...
def JSD(conversion, y_train, y_train_pred, w_train, tight_train, iso_train, y_test, y_test_pred, w_test, tight_test, iso_test, plot_type, save_directory):
    ...
    fig_fail.savefig(f"/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{save_directory}/JSD_fail.pdf", bbox_inches='tight')
    plt.close(fig_fail)
    fig_pass.savefig(f"/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{save_directory}/JSD_pass.pdf", bbox_inches='tight')
    plt.close(fig_pass)