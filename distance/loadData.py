...
# create logger
logger = logging.getLogger('runDistance.loadInput')

class DataLoader(object):
    '''
    Class loading the input data for the photon ID Distance network
    '''
    def __init__(self, file):
        ...
        self.use_zeros = self.config[section].getboolean('use_zeros')

        if not os.path.exists('/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{}'.format(self.save_directory)):
            os.makedirs('/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{}'.format(self.save_directory))
        
        newPath = shutil.copy(file, '/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{}'.format(self.save_directory))
    ...
    def dataPreprocessing(self):
        ...
        """
        # add decorrelation weights
        self.sgn_df['dcor_weight'] = 0
        if (self.use_zeros == False):
            self.bkg_df['dcor_weight'] = self.bkg_df['train_weight'] * ((self.bkg_df['y_ptcone40']==0.0)*0 + (self.bkg_df['y_ptcone40']!=0)*1)
            #self.sgn_df['dcor_weight'] = self.sgn_df['train_weight'] * ((self.sgn_df['y_ptcone40']==0.0)*0 + (self.sgn_df['y_ptcone40']!=0)*1)
        elif (self.use_zeros == True):
            self.bkg_df['dcor_weight'] = self.bkg_df['train_weight']
            #self.sgn_df['dcor_weight'] = self.sgn_df['train_weight']
        
        # add aditional weights for the "network fit function"
        self.sgn_df['iso_weight'] = 1
        self.bkg_df['iso_weight'] = 1
        ...
        #adding the FixedCutTight condition ass boolean
        self.tight_pass = self.data.loc[((self.data['y_topoetcone40']) < ((self.data['y_pt'])*0.022)+2.45) & ((self.data['y_ptcone20']) < (0.05*(self.data['y_pt'])))]
        self.tight_fail = self.data.loc[((self.data['y_topoetcone40']) >= ((self.data['y_pt'])*0.022)+2.45) | ((self.data['y_ptcone20']) >= (0.05*(self.data['y_pt'])))]       
        self.tight_pass['track'] = 1.0
        self.tight_fail['track'] = 0.0

        self.data = pd.concat([self.tight_fail, self.tight_pass])

        '''
        fail = self.tight_fail.to_numpy()                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           
        pas = self.tight_pass.to_numpy()

        # merge pass and fail
        data = np.zeros((self.tight_pass.shape[0]+self.tight_fail.shape[0],self.tight_pass.shape[1]))
        lentgh_pass = len(pas)
        lentgh_fail = len(fail)
        for idx in range(0, lentgh_pass):
            data[idx] = pas[idx]
        for idx in range(0, lentgh_fail):
            data[idx+lentgh_pass] = fail[idx]
        self.data = pd.DataFrame(data, columns=self.tight_pass.values.tolist())
        '''
        # shuffle and split
        ...
        self.branches_root = ['class', 'y_pt', 'y_eta', 'y_isTruthMatchedPhoton', 'y_ptcone20', 'y_ptcone40', 'y_topoetcone20', 'y_topoetcone40', 'y_IsLoose', 'y_IsTight', 'y_convType', 'dataWeightPtBin', 'evt_mu', 'track'] 
        ...
        if (self.save_scaler):
            joblib.dump(self.scaler, f"/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{self.save_directory}/scaler.gz")
        ...
        # define decorrelation target and weight
        # the ney target vector contains the weight for distance correlation (the additional weight calles w_train/test_iso is just 1 for every event)
        y_train_iso, w_train_iso = self.train_data[['track', 'dcor_weight']], self.train_data['iso_weight']
        y_test_iso, w_test_iso = self.test_data[['track', 'dcor_weight']], self.test_data['iso_weight']
        ...
        self.train_dict = {
            ...
            "y_train_iso": y_train_iso.to_numpy(),
            "w_train_iso": w_train_iso.to_numpy(),
        ...
        self.test_dict = {
            ...
            "y_test_iso": y_test_iso.to_numpy(),
            "w_test_iso": w_test_iso.to_numpy(),
