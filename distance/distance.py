...
import plotDistance as plot
import prePlotDistance as prePlot
import saveResults as save
import time
from lossDistance import distance_corr_weighted, distance_corr 
...
# create logger
logger = logging.getLogger('runDistance.Distance')

class Distance(object):
    '''
    Class implementing an Distance used for photon ID
    '''
    def __init__(self, file, train_dict, test_dict, root_variables):
        ...
        if (self.training_mode == "normal"):
            ...
            self.classifier_pre_training = self.config[section].getboolean('classifier_pre_training')
            self.classifier_pre_epochs = int(self.config[section]['classifier_pre_epochs'])
        ...
        # decorrelation variables 
        section = "decorrelation"
        self.classifier_lambda_value = float(self.config[section]['classifier_lambda_value'])
        ...
        if (self.training_mode == "optimize"):
            ...
            lambda_value = self.config[section]['lambda_value'].split(',')
            self.lambda_value = [float(i) for i in lambda_value]
        ...
        self.y_train_iso = train_dict['y_train_iso']
        self.w_train_iso = train_dict['w_train_iso']
        ...
        self.y_test_iso = test_dict['y_test_iso']
        self.w_test_iso = test_dict['w_test_iso']
        ...
        self.iso_train = self.root_train["track"]
        self.iso_test = self.root_test["track"]
...
###########################################
    ...
    def initializeDistance(self):
        """
        Initalize the distance correlation classifier with the config defined settings
        """

        def disCoWeighted(y_true, y_pred):
            weights = y_true[:,1]
            y_true = y_true[:,0]
            
            return distance_corr_weighted(y_true, y_pred, weights)
        ...
        self.model_classifier = Model(inputs = [self.network_input], outputs = [self.layer_classifier_out, self.layer_classifier_out], name="classifier_training")
        self.model_classifier_pre_train = Model(inputs = [self.network_input], outputs = [self.layer_classifier_out], name="classifier_pre_training")
        
        self.model_classifier.compile(loss = ['binary_crossentropy', disCoWeighted], loss_weights=[1, self.classifier_lambda_value], optimizer = self.classifier_optimizer, run_eagerly=True)
        self.model_classifier_pre_train.compile(loss='binary_crossentropy', optimizer = self.classifier_optimizer)

        logger.info('Distance classifier built with the following structure:')
...
###########################################
    ...
    def preTrainDistance(self):
        '''
        Pre-training the classifier model.
        '''
        self.callbacks = None
        ...
        logger.info(f'Pre-training classifier with {self.classifier_pre_epochs} epochs.')
        self.classifier_pre_history = self.model_classifier_pre_train.fit(  x = self.x_train_classifier, y = self.y_train_classifier, sample_weight=self.w_train_classifier,
                                                                    validation_data = (self.x_test_classifier, self.y_test_classifier, self.w_test_classifier),
                                                                    epochs=self.classifier_pre_epochs, batch_size = self.classifier_batch_size, 
                                                                    callbacks = self.callbacks ,verbose = self.verbose
                                                                    )
                                                                       
        logger.info("Finished pre-training the classifier")

    def trainDistance(self):
        '''
        Training the distance classifier model.
        '''
        self.callbacks = None
        # early stopping callback
        if (self.classifier_early_stopping):
        ...
        self.classifier_history = self.model_classifier.fit(    x = self.x_train_classifier, y = [self.y_train_classifier, self.y_train_iso], 
                                                                sample_weight=[self.w_train_classifier, self.w_train_iso],
                                                                validation_data = (self.x_test_classifier, [self.y_test_classifier, self.y_test_iso], 
                                                                [self.w_test_classifier, self.w_test_iso]),
    ...
    def trainOptimizeDistance(self):
        '''
        Run the parameter optimization.
        '''
        def initalizeOptimizeDistance(hp):
            """
            Initalize the distance correlation classifier with the config defined settings for bayesian optimization
            """
            def disCoWeighted(y_true, y_pred):
                return distance_corr(y_true, y_pred)
            ...
            hp_lambda_value = hp.Choice("lambda_value", values=self.lambda_value)
            ...
            model_classifier = Model(inputs = [network_input], outputs = [layer_classifier_out, layer_classifier_out], name="classifier_training")
            model_classifier.compile(loss = ['binary_crossentropy', disCoWeighted], loss_weights=[1, hp_lambda_value], optimizer = classifier_optimizer, run_eagerly=True)
        ...
        # early stopping callback
        if (self.classifier_early_stopping):
            self.callbacks = EarlyStopping(monitor='val_loss', mode="min", patience=4)

        self.terminate = TerminateOnNaN()
        tuner = kt.BayesianOptimization( initalizeOptimizeDistance,
                                         objective=kt.Objective('val_loss', direction="min"),
                                         num_initial_points=5,
                                         max_trials=self.num_steps,
                                         directory=f'/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{self.save_directory}/',
                                         project_name='bayesOpt')
                                            
        tuner.search(   x = self.x_train_classifier, y = [self.y_train_classifier, self.y_train_iso], 
                        sample_weight=[self.w_train_classifier, self.w_train_iso],
                        validation_data = (self.x_test_classifier, [self.y_test_classifier, self.y_test_iso], 
                        [self.w_test_classifier, self.w_test_iso]),
                        epochs=self.classifier_epochs, batch_size = self.classifier_batch_size,  
                        callbacks = [self.callbacks, self.terminate], verbose = self.verbose
                        )
        ...
        logger.info(f"""The hyperparameter search is complete.
                ...
                Optimizer lambda value: {best_hps.get('lambda_value')} 
                """)

        # save values to txt file
        text_file = open(f"/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{self.save_directory}/hyperparameters.txt", "w")
...
###########################################
    ...
    def prePredModels(self):
        '''
        Predicting the pre trained classifier model.
        '''
        logger.info("Prediciting classifier output after training")
        # pred classifier
        self.y_train_classifier_pre_pred = self.model_classifier_pre_train.predict(self.x_train_classifier)
        self.y_test_classifier_pre_pred = self.model_classifier_pre_train.predict(self.x_test_classifier)
    ...
    def predModels(self):
        ...
        logger.info("Prediciting classifier output after training")
        # pred classifier
        self.y_train_classifier_pred, placeholder = self.model_classifier.predict(self.x_train_classifier)

        self.y_test_classifier_pred, placeholder = self.model_classifier.predict(self.x_test_classifier)
        if (self.save_model):
            if not os.path.exists('/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{}/model/'.format(self.save_directory)):
                os.makedirs('/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{}/model/'.format(self.save_directory))
            logger.info("Saving the model trained model in a sub directory")
            self.model_classifier.save('/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{}/model/classifier_model'.format(self.save_directory))


    def prePlotResults(self):
        '''
        Plot the results of the pre training
        '''
        logger.info("Pre-training plots are being created and saved in the /plots/{}/pre/ sub-directory".format(self.save_directory))
        if not os.path.exists('/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{}/pre'.format(self.save_directory)):
            os.makedirs('/cephfs/user/s6herose/Bachelorarbeit/Programmieren/codemasterthesis/finalScripts/distance/plots/{}/pre'.format(self.save_directory))
        prePlot.Loss(self.conversion_type, self.classifier_pre_history, self.save_directory)
        prePlot.classifierPredictions(self.conversion_type, self.y_train_classifier, self.y_train_classifier_pre_pred, self.w_original_train, 
                                   self.y_test_classifier, self.y_test_classifier_pre_pred, self.w_original_test, 
                                   self.plot_type, self.save_directory) 
        prePlot.ROC(self.conversion_type, self.y_train_classifier, self.y_train_classifier_pre_pred, self.w_original_train,
                 self.y_test_classifier, self.y_test_classifier_pre_pred, self.w_original_test,
                 self.plot_type, self.save_directory) 
        prePlot.Efficiency(self.conversion_type, self.y_train_classifier, self.y_train_classifier_pre_pred, self.w_original_train, self.isTight_train, 
                        self.y_test_classifier, self.y_test_classifier_pre_pred, self.w_original_test, self.isTight_test,
                        self.save_directory)
        prePlot.JSD(self.conversion_type, self.y_train_classifier, self.y_train_classifier_pre_pred, self.w_original_train, self.isTight_train, self.iso_train,
                    self.y_test_classifier, self.y_test_classifier_pre_pred, self.w_original_test, self.isTight_test, self.iso_test,
                    self.plot_type, self.save_directory)
        logger.info("Plotting classifier output after pre-training done")
    ...
    def plotResults(self):
        ...
        plot.Loss(self.conversion_type, self.classifier_history, self.classifier_lambda_value, self.save_directory)
